...
def render_random_sample_export():
    ...
    # Display total shape of uploaded data
    st.write(f"Total Data Shape: {df.shape}")

    # Sample size input
    sample_size = st.number_input("Enter Sample Size", min_value=1, max_value=df.shape[0], value=min(10, df.shape[0]), step=1)

    # Display random sample
    if st.button("Display Random Sample"):
    ...
    autogenerated_sample_name = f"Random_sample_from_{uploaded_file.name}}"
    ...
    if st.button("Export Sample"):
        ...
        st.success(f"Export successful! Navigate to Exported Files for download")
